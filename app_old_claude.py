...
with tab_input:
    ...
    with col_R:
        with st.container(border=True):
            ...
            eval_month = int(report_month.replace("월", ""))
            ...
            for i in range(12):
                m_label = f"{i+1}월"
                # 평가 월: student_score 기본값, 나머지: 0
                default_v = float(student_score) if (i+1)==eval_month else 0.0
                ...
                if val > 0:
                    ...
                    q_scores.append(val)
            if not q_scores:   # 모두 0이면 평가 월 기본
                q_labels = [f"{eval_month}월"]
